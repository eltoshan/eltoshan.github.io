...
From a Bayesian point of view, bandits with binary outcomes $x$ can be easily modeled using the beta-binomial predictive distribution. That is,
...
And the posteriod predictive distribution for the probability of success $p$, given $n$ trials and $x$ successes, becomes  

$$ p|n,x,\alpha,\beta \sim Beta(\alpha + x, \beta + n - x) $$
